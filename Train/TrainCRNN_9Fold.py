# ==========================================================
# CRNNモデル (CNN + BiLSTM) の 9分割交差検証コード
# ==========================================================
...
from tensorflow.keras.layers import (Input, Conv2D, BatchNormalization, MaxPooling2D,
                                     Activation, Reshape, Bidirectional, LSTM, Dense, Dropout)
...
# --- 1. 設定・パラメータ ---
DATA_DIR = Path('../data/segmented_data_split/train')  # 全データをここにまとめる
SAMPLE_RATE = 16000
DURATION = 1.5
...
for i, class_name in enumerate(CLASS_NAMES):
    for wav_path in (DATA_DIR / class_name).glob('*.wav'):
        ...
        log_melspec = np.expand_dims(log_melspec, axis=-1)  # (128, time, 1)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), start=1):
    ...
    model = build_crnn_model(X.shape[1:], NUM_CLASSES)

    callbacks = [
        EarlyStopping(monitor='val_loss', patience=50, restore_best_weights=True),
        ModelCheckpoint(f"crnn_fold{fold:02d}.keras", monitor='val_loss', save_best_only=True)
    ]
